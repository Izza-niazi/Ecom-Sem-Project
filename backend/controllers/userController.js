...
exports.registerUser = asyncErrorHandler(async (req, res, next) => {
    ...
    if (rawAvatar && String(rawAvatar).trim() !== "") {
        try {
            const myCloud = await uploadImage(rawAvatar, { folder: "avatars" });
            avatar = {
                public_id: myCloud.public_id,
                url: myCloud.secure_url,
            };
        } catch (err) {
            console.warn("Avatar upload failed (using default):", err.message);
...
exports.updateProfile = asyncErrorHandler(async (req, res, next) => {
    ...
    if (req.body.avatar && String(req.body.avatar).trim() !== "") {
        const user = await User.findById(req.user.id);
        const imageId = user.avatar && user.avatar.public_id;
        try {
            const myCloud = await uploadImage(req.body.avatar, { folder: "avatars" });
            newUserData.avatar = {
                public_id: myCloud.public_id,
                url: myCloud.secure_url,
            };
