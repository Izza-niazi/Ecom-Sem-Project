...
exports.getAllProducts = asyncErrorHandler(async (req, res, next) => {
  ...
  let searchFeature = new SearchFeatures(Product.find(), q).search().filter();

  let products = await searchFeature.query;
  let filteredProductsCount = products.length;
  let searchRelaxed = false;
  let searchRelaxedMode = null;

  // NL/LLM often picks a wrong category or brand → zero rows. Retry keyword + price/ratings only.
  const kw = q.keyword && String(q.keyword).trim();
  if (filteredProductsCount === 0 && kw && (q.category || q.brand)) {
    const relaxed = { ...q };
    delete relaxed.category;
    delete relaxed.brand;
    const relaxedFeature = new SearchFeatures(Product.find(), relaxed)
      .search()
      .filter();
    const relaxedProducts = await relaxedFeature.query;
    if (relaxedProducts.length > 0) {
      searchFeature = relaxedFeature;
      products = relaxedProducts;
      filteredProductsCount = relaxedProducts.length;
      searchRelaxed = true;
      searchRelaxedMode = "drop_category_brand";
    }
  }

  // Vague phrases ("something to wear") almost never appear in titles. Browse category + price/ratings only.
  if (filteredProductsCount === 0 && kw && q.category) {
    const browse = { ...q };
    delete browse.keyword;
    const browseFeature = new SearchFeatures(Product.find(), browse)
      .search()
      .filter();
    const browseProducts = await browseFeature.query;
    if (browseProducts.length > 0) {
      searchFeature = browseFeature;
      products = browseProducts;
      filteredProductsCount = browseProducts.length;
      searchRelaxed = true;
