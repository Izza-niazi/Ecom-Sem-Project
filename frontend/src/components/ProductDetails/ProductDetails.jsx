...
import { clearErrors, getProductDetails, getSimilarProducts, newReview, trackProductActivity } from '../../actions/productAction';
...
const ProductDetails = () => {
    ...
    useEffect(() => {
        ...
        dispatch(trackProductActivity(productId, 'view', 'product_detail'));
    ...
    let metaEl;
    if (loading || !product) {
        metaEl = <MetaData title={metaTitle('Product')} />;
    } else {
        const seo = product.seo || {};
        const pageTitleText = (seo.pageTitle && seo.pageTitle.trim()) || product.name;
        const metaDesc =
            (seo.metaDescription && seo.metaDescription.trim()) ||
            excerpt(plainTextFromHtml(product.description), 160);
        const kw =
            (seo.keywords && seo.keywords.trim()) ||
            [product.name, product.category, product.brand?.name].filter(Boolean).join(', ');
        const ogTitle = (seo.ogTitle && seo.ogTitle.trim()) || pageTitleText;
        const ogDescription =
            (seo.ogDescription && seo.ogDescription.trim()) || metaDesc;
        const path =
        ...
        metaEl = (
            <MetaData
                title={metaTitle(pageTitleText)}
                description={metaDesc}
                keywords={kw}
                robots={robots}
                canonical={canonical}
                ogTitle={metaTitle(ogTitle)}
                            ...
                            <div className="w-full sm:w-2/5 sm:sticky top-16 sm:h-screen">
                                ...
                                <div className="flex flex-col gap-3 m-3">
                                    <div className="w-full h-full pb-6 border relative">
                                        ...
                                        <div className="absolute top-4 right-4 shadow-lg bg-app-card w-9 h-9 border flex items-center justify-center rounded-full">
                                ...
                                <div className="flex flex-col gap-2 mb-4">
                                    ...
                                    <div className="w-full mt-4 pb-4 rounded-sm border flex flex-col">
                                        ...
                                        {product.specifications?.map((spec, i) => (
                                            <div className="px-6 py-2 flex items-center text-sm" key={i}>
                                                ...
                                                <p className="flex-1">{spec.description?.trim() ? spec.description : '—'}</p>
                                            </div>
                                        ))}
                                        {/* <!-- specs list --> */}
                                    ...
                                    <div className="w-full mt-4 rounded-sm border flex flex-col">
                                        ...
                                        >
                                            ...
                                            <DialogActions>
                                                <button onClick={handleDialogClose} className="rounded border border-red-500/80 bg-app-card py-2 px-6 uppercase text-red-400 shadow hover:bg-red-950/40">Cancel</button>
                        ...
                        <div className="mt-6 flex flex-col gap-3">
                            {alsoBought.length > 0 ? (
                                <ProductSlider
                                    ...
                                    tagline="Often purchased in the same order"
                                    externalProducts={alsoBought}
                                    sectionIndex={0}
                                    variant="slice"
                                />
                            ) : null}
