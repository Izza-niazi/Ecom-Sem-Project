...
const OrderItem = (props) => {
    ...
    return (
        <Link to={`/order_details/${orderId}`} className="flex items-start gap-2 rounded-md border border-app-border bg-app-card p-4 hover:shadow-lg hover:shadow-black/25 sm:gap-0">
            {/* <!-- image container --> */}
            ...
            <div className="flex flex-col sm:flex-row justify-between w-full">
                ...
                <div className="flex flex-col gap-1 overflow-hidden">
                    ...
                    <p className="text-xs text-gray-500">Total: {formatRs(quantity * price)}</p>
                </div>

                <div className="flex flex-col sm:flex-row mt-1 sm:mt-0 gap-2 sm:gap-20 sm:w-1/2">
                    <p className="text-sm">{formatRs(price)}</p>
